...
This program creates a class of type "Network" and this time
starts off as false (so no one is following each other)
and then if both usernames are in the network make usern1 follow 
usern2 and thus it would return true if successful 
otherwise if neither of those usernames exist then it returns false 
and then it is displayed in the dot file format that displays the 
